...
function nppp_enqueue_nginx_fastcgi_cache_purge_preload_assets() {
    ...
    wp_enqueue_style('nppp_admin-css', plugins_url('../admin/css/fastcgi-cache-purge-and-preload-nginx.min.css', __FILE__), array(), '2.0.5');
    ...
    */
    ...
    wp_enqueue_script('nppp_admin-js', plugins_url('../admin/js/fastcgi-cache-purge-and-preload-nginx.min.js', __FILE__), array('jquery'), '2.0.5', true);
...
function nppp_enqueue_nginx_fastcgi_cache_purge_preload_requisite_assets() {
    ...
    if ($nppp_met) {
        ...
        if (empty($output)) {
            ...
            wp_enqueue_script('preload-button-disable', plugins_url('../admin/js/preload-button-disable.js', __FILE__), array('jquery'), '2.0.5', true);
        } else {
        ...
        wp_enqueue_script('nppp-disable-functionality', plugins_url('../admin/js/nppp-disable-functionality.js', __FILE__), array('jquery'), '2.0.5', true);
    }
}
...
function nppp_enqueue_nginx_fastcgi_cache_purge_preload_front_assets() {
    if (is_user_logged_in() && current_user_can('administrator') && isset($_GET['nppp_front'])) {
        ...
        if (wp_verify_nonce($nonce, 'nppp_redirect_nonce')) {
            if (!is_admin()) {
                // Enqueue CSS files for Nginx FastCGI Cache Purge and Preload Plugin
                wp_enqueue_style('nppp_admin-front-css', plugins_url('../frontend/css/fastcgi-cache-purge-and-preload-nginx-front.css', __FILE__), array(), '2.0.5');
                // Enqueue JavaScript files for Nginx FastCGI Cache Purge and Preload Plugin frontend
                wp_enqueue_script('nppp_admin-front-js', plugins_url('../frontend/js/fastcgi-cache-purge-and-preload-nginx-front.js', __FILE__), array('jquery'), '2.0.5', true);
            }
        }
    }
    ...
    $nppp_met = nppp_plugin_requirements_met();
    if (!$nppp_met) {
        wp_enqueue_script('nppp-disable-functionality-front', plugins_url('../frontend/js/nppp-disable-functionality-front.js', __FILE__), array('jquery'), '2.0.5', true);
    } else {
...
function nppp_manage_admin_notices() {
    ...
    wp_register_style('nppp-manage-notices', false, array(), '2.0.5');
